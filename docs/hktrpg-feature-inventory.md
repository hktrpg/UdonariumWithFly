...
| Guided tour | First-run overlay: rooms & save, menus, table gestures, shortcuts; welcome language picker; skip/replay from Settings | `guided-tour.service.ts`, `guided-tour-steps.ts`, `guided-tour.component.*`, i18n `*-tip.ts` | Fresh profile shows welcome → complete/skip; Settings replay restarts |
