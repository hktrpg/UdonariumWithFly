...
| Multi-track BGM | Up to 4 concurrent room tracks + local ambience; audition is local-only; **max 20MB per audio file** | `Jukebox.ts`, `jukebox.component.*`, `file-archiver.ts` | Play multiple tracks; mute audition does not broadcast; >20MB rejected |
