...
完整说明：https://bothelp.hktrpg.com/guide`,
  'tutorial.view': `＜视角＞Shift＋左键拖曳＝平移地图　右键拖曳＝旋转视角　滚轮＝缩放
...
＜路径移动＞先选取 Token，按住 Shift＋左键点地图加路点，再 Shift＋右键依序移动
　　　　放开 Shift（未右键确认）＝取消路点
＜选取＞左键点物件＝选取（高亮）　Ctrl＋点＝加减选
　　　　左键空白拖曳＝框选　点空白／Esc＝取消选取
图片、音乐可直接拖进浏览器导入。`,
  'tutorial.keyboard': `＜键盘（选取后）＞WASD／方向键＝移动（可对角）
　　　　Shift＋WASD＝改变面向　Delete＝删除（角色进回收区）
　　　　Ctrl＋C／X／V＝复制／剪切／粘贴（贴在鼠标光标处；选取文字时仍可复制文字）
　　　　[ ＝送到后层　] ＝送到前层
　　　　Alt＋滚轮＝旋转 15°　Ctrl＋Shift＋滚轮＝旋转 45°
...
　　　　棋子・遮罩・地形默认挡光；右键可关「与灯光互动」
　　　　状态图标常驻于名牌；套圈可在右键「套圈」选择
＜场景工具＞仅 GM（菜单）；选取／灯光／墙壁／矩形／椭圆／多边形／手绘／文字
　　　　选取后：WASD／方向键移动　Delete 删除；墙・多边形：Enter／双击完成；Esc 取消
...
・桌面选取高亮；左键空白框选；Shift＋左键平移地图
・双击物件开启详情（翻面／抽牌／掷骰改右键菜单）
・键盘：WASD 移动、Shift＋WASD 面向、Delete、Ctrl＋C／X／V（贴在光标）
・[ / ] 调整前后层；Alt＋滚轮 15°／Ctrl＋Shift＋滚轮 45° 旋转
・Shift＋滚轮左右移动画面；Ctrl＋滚轮上下移动画面
・路径移动：选取 Token → Shift＋左键加路点 → Shift＋右键移动；放开 Shift 取消
