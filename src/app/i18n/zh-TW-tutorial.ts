...
　　　　拖旋轉把手＝轉向
　　　　右鍵＝選單
　　　　雙擊＝開啟詳情（角色／卡牌／牌堆／骰子／地形／筆記／遮罩／範圍等；反面卡對玩家不開）
　　　　角色「下一張圖像」＝右鍵選單「切換下一張圖像」
　　　　卡牌／牌堆：右鍵抽入手牌・派牌；F＝單卡翻面／牌堆整疊翻轉；骰子＝右鍵或 F
...
　　　　Shift＋拖曳＝加選框選
　　　　點空白／Esc＝取消選取
＜預覽＞角色等＝滑鼠停上顯示概要（可釘選；移開約 0.5 秒淡出）
　　　　卡牌／牌堆＝Hover 顯示名稱→text
　　　　Ctrl／⌘＋懸停＝物件大圖預覽（縮放／釘選；雙擊開詳情）
圖片、音樂可直接拖進瀏覽器匯入。`,
...
・共用筆記支援文字（Markdown）／圖片／影片（≤50MB）／PDF（≤50MB）；可拖曳匯入與切換顯示內容
...
・第二分頁入房後 TOKEN／圖片較不易空白（settle＋延遲 remount）`,
  'changelog.2026mesh2': `2026/08/22–23 連線・mesh・PWA
...
・PWA：ngsw 雜湊與 HKTRPG 建置對齊；SW 安裝失敗也顯示更新圖示；連線面板捲動不再跳位`,
  'changelog.2026filesnote': `2026/08/22–23 檔案同步・筆記倉庫
...
・紅繩（線索連結）拖曳連線物件時即時更新；預設劇本文字不再重複載入`,
  'changelog.2026sepStability': `2026/08/24–31 穩定・連線・光照・聊天
・地圖**黑暗**與**環境光**分開；日／夜預設與夜晚地圖邊緣密封更準
・入房 probe／弱網 ICE：慢速連線較不易誤判逾時；mesh 診斷含房間媒體用量
・PDF：worker 以 blob 載入（避開錯誤 MIME）；停止渲染重試風暴卡死 Chrome
・切換身份時保留角色密碼；大廳入房 UI 提示更清楚
・進房／重開房：不再清掉已有聊天分頁；拒收 peer 重造大廳 MainTab／SubTab；lobby sample DELETE 不再回推`,
  'changelog.2026handcard': `2026/09/01 手牌庫・牌堆・物件預覽
・底部**手牌庫**：直排 tuck（露出頂部）、hover 展開；可拖曳重排／打出；打出時可選正面／背面
