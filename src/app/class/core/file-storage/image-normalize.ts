...
export async function normalizeImageBlob(blob: Blob): Promise<NormalizeImageResult> {
  ...
  let out = await encodeImageToMaxEdge(img, w, h, TARGET_MAX_EDGE, hasAlpha, JPEG_QUALITY);

  if (out.size > IMAGE_STORED_MAX_BYTES) {
    out = await encodeImageToMaxEdge(img, w, h, FALLBACK_MAX_EDGE, hasAlpha, JPEG_QUALITY_FALLBACK);
  }
  ...
  if (out.size > IMAGE_STORED_MAX_BYTES && hasAlpha) {
    ...
    for (const edge of [1024, 768, 512, 384, 256]) {
      out = await encodeImageToMaxEdge(img, w, h, edge, true, JPEG_QUALITY);
      if (out.size <= IMAGE_STORED_MAX_BYTES) break;
    }
  } else if (out.size > IMAGE_STORED_MAX_BYTES) {
    out = await encodeImageToMaxEdge(img, w, h, FALLBACK_MAX_EDGE, false, 0.65);
  }
